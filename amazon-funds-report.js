...
function analyse(invoices, { snowOnly = true, seasonKey = null, bu = null } = {}) {
  ...
  // ── POs carrying invoices from a site other than their own ────────────────
  // NOT evidence of a multi-site PO, though it was briefly used that way.
  // Edwin produced the actual PO documents for 2D-21907390 and 2D-21976622 on
  // 2026-09-14: each has ONE ship-to and ONE line item naming ONE site. The
  // extra site codes we see against them are OUR invoices billed to the wrong
  // purchase order, not Amazon consolidating sites.
  //
  // The distinction matters because the deck nearly asserted the opposite to
  // Amazon. A PO's scope can only be read off the PO document; what we bill
  // against it says nothing about what it covers. So this list is now surfaced
  // as a BILLING EXCEPTION, and nothing derives a claim about PO scope from it.
  const crossBilled = (() => {
    let rows2 = [];
    ...
    for (const p of ledger) {
      ...
      out.push({
        ...
        sites: [...new Set(foreign.map(r => r.site))].sort(),
      });
    }
